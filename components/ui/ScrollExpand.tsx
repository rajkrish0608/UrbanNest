...
const ScrollExpand = ({
  ...
  const propsRef = useRef({ startWidth, startHeight, startRadius, endRadius, mediaZoom, scrollDistance, holdDistance, smoothing, overlayScrim, useWindowScroll, enabled });
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      target = readProgress();
      if (smoothing <= 0 || reduceMotion) { current = target; applyProgress(current); return; }
  ...
  }, [applyProgress, useWindowScroll, smoothing]);
      ...
      <div ref={trackRef} style={{ position:'relative', width:'100%' }}>
        <div ref={stageRef} style={{ position:'sticky', top:0, width:'100%', overflow:'hidden' }}>
          <div ref={frameRef} style={{ position:'absolute', inset:0, clipPath:`inset(21% 29% 21% 29% round 24px)`, willChange:'clip-path' }}>
            ...
            <div ref={scrimRef} style={{ position:'absolute', inset:0, pointerEvents:'none', background:'linear-gradient(to top, rgba(20,16,11,0.85), rgba(20,16,11,0.1) 45%, rgba(20,16,11,0.35))', opacity:0 }} />
            {children && (
              <div ref={overlayRef} style={{ position:'absolute', inset:0, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', textAlign:'center', padding:'6%', opacity:0, willChange:'opacity,transform' }}>
                {children}
              </div>
            )}
          </div>
          {title && (
            <div ref={titleRef} style={{ position:'absolute', inset:0, display:'flex', alignItems:'center', justifyContent:'center', padding:'0 6%', textAlign:'center', fontSize:'clamp(2rem,6vw,5rem)', fontWeight:700, letterSpacing:'-0.03em', lineHeight:1, color:'var(--color-cream)', textShadow:'0 2px 24px rgba(0,0,0,0.45)', pointerEvents:'none', willChange:'opacity,transform' }}>
              {title}
            </div>
          )}
          {scrollHint && (
            <div ref={hintRef} style={{ position:'absolute', left:0, right:0, bottom:'1.25rem', textAlign:'center', fontSize:'0.8125rem', letterSpacing:'0.02em', color:'rgba(255,237,215,0.55)', pointerEvents:'none', willChange:'opacity,transform' }}>
              {scrollHint}
            </div>
          )}
        </div>
